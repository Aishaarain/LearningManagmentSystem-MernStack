  ...
  setChapters(
    chapters.map((chapter) => {
      if (chapter.chapterId === currentChapterId) {
        ...
        // Return a NEW chapter object with updated lectures
        return {
          ...chapter,
  ...
  {chapters.map((chapter, chapterIndex) => (
    <div key={chapterIndex} className="bg-white border rounded-lg mb-4">
      <div className="flex justify-between items-center p-4 border-b">
        ...
        <img src={assets.cross_icon} className='cursor-pointer' alt="" onClick={()=> handleChapter('remove', chapter.chapterId)} />
      </div>
...
<img onClick={()=>handleLecture('remove', chapter.chapterId, lectureIndex)} src={assets.cross_icon} alt="" className='cursor-pointer'/>
  ...
  + Add Lecture
      ...
      {/* Chapter content (e.g., lectures) would be rendered here, potentially conditionally based on chapter.collapsed */}
    </div>
  ))}
...
<img  className='absolute top-4 right-4 w-4 cursor-pointer'
 src={assets.cross_icon} onClick={()=>setShowPopup(false)} alt="" />
    </div>
  </div>
)}
</div>
